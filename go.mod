module github.com/psanford/cloudip

go 1.24.0

toolchain go1.24.1

require (
	github.com/psanford/awsip v0.0.0-20260205075837-e23ea53b527b
	github.com/psanford/cloudflareip v0.0.0-20240811072658-acd6b0d934b9
	github.com/psanford/gcpip v0.0.0-20260201075010-ba8ab097d414
	github.com/psanford/githubip v0.0.0-20260205075600-d9a1fe8ff909
)

require (
	github.com/gaissmai/cidrtree v0.5.0 // indirect
	github.com/gaissmai/extnetip v1.3.1 // indirect
	golang.org/x/mod v0.32.0 // indirect
	golang.org/x/sync v0.19.0 // indirect
	golang.org/x/tools v0.41.0 // indirect
)
