module github.com/psanford/cloudip

go 1.25.0

require (
	github.com/psanford/awsip v0.0.0-20260609104241-4ee0e92f0cda
	github.com/psanford/cloudflareip v0.0.0-20240811072658-acd6b0d934b9
	github.com/psanford/gcpip v0.0.0-20260531095742-014293d76694
	github.com/psanford/githubip v0.0.0-20260609103954-271318578730
)

require (
	github.com/gaissmai/cidrtree v0.5.0 // indirect
	github.com/gaissmai/extnetip v1.3.1 // indirect
	golang.org/x/mod v0.37.0 // indirect
	golang.org/x/sync v0.21.0 // indirect
	golang.org/x/tools v0.45.0 // indirect
)
